...
export default function EditorToolbar({ editor, disabled }: Props) {
  ...
  const btn = (key: string, { label, title, action, isActive }: ToolButton) => (
    <button
      key={key}
      title={title}
      onClick={action}
  ...
  const divider = (key: string) => (
    <span key={key} className="w-px h-5 bg-[var(--border-strong)] mx-1" />
  );

  return (
    <div className="flex flex-wrap items-center gap-0.5 px-4 py-2 border-b border-[var(--border)] bg-[var(--bg-toolbar)]">
