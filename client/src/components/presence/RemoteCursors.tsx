...
export default function RemoteCursors({
  ...
  const draw = useCallback(() => {
    ...
    remoteUsers.forEach(({ user, cursor }) => {
      if (!cursor) return;
      const { anchor, head } = cursor as { anchor: number; head: number };
      ...
      if (anchor !== head) {
        ...
        lines.forEach((line) => {
          ...
          highlight.style.cssText = `position:absolute;top:${c1.top}px;left:${c1.left}px;width:${Math.max(c2.left - c1.left, 4)}px;height:${c1.lineHeight}px;background:${color};opacity:0.2;border-radius:2px;`;
          canvas.appendChild(highlight);
        });
      }

      const caret = document.createElement("div");
      caret.style.cssText = `position:absolute;top:${coords.top}px;left:${coords.left}px;width:2px;height:${coords.lineHeight}px;background:${color};border-radius:1px;animation:yjsCursorBlink 1.2s step-end infinite;`;
      canvas.appendChild(caret);

      const badge = document.createElement("div");
      badge.textContent = user.name;
      badge.style.cssText = `position:absolute;top:${coords.top - 22}px;left:${coords.left}px;background:${color};color:#fff;font-size:11px;font-weight:500;padding:2px 6px;border-radius:4px;white-space:nowrap;pointer-events:none;opacity:0;animation:yjsBadgeFade 2.5s ease forwards;`;
  ...
  useEffect(() => {
    ...
    style.id = id;
    style.textContent = `@keyframes yjsCursorBlink{0%,100%{opacity:1}50%{opacity:0}}@keyframes yjsBadgeFade{0%{opacity:1;transform:scale(1)}60%{opacity:1}100%{opacity:0;transform:scale(0.95)}}`;
